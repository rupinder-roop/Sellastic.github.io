...
function registercheck() {
    ...
    var cpass = $("#register-Cpassword").val();
    var email = $("#register-email").val();
    var checkotpvalid= $('#otp').val();

    var checkit = /^(?=.*[0-9])(?=.*[!@#$%^&*])[A-Za-z!@#$%^&*0-9]{8,20}$/;
    ...
    if (pass == "") {
        Swal.fire({
            ...
            text: "Must Contains Password",
            showConfirmButton: false,
            timer: 1500
        });

    }else if(checkotpvalid == ""){
        Swal.fire({
            position: 'center',
            icon: 'error',
            title: 'Invalid Condition',
            text: "Must Contains OTP",
            showConfirmButton: false,
            timer: 1500
        });
    }else if (pass.match(checkit)) {
        if (pass == cpass) {
            var logindetails = JSON.parse(localStorage.getItem("LoginDatainfo"));
            if(logindetails){
                console.log(logindetails);
                if(checkusername(logindetails,user)){
                    ...
                    logindetails.push({ username: user, password: pass, email: email ,address:""});
                    localStorage.setItem("LoginDatainfo", JSON.stringify(logindetails));
                    currentLogin.push({username:user});
                    localStorage.setItem("currentLoginInfo",JSON.stringify(currentLogin));
                }
            }else{
                var len = LoginData.length;
                for (let i = 0; i < len; i++) {
                    var LoginDatacopy = LoginData;
                }
                LoginDatacopy.push({ username: user, password: pass, email: email ,address:""});
                localStorage.setItem("LoginDatainfo", JSON.stringify(LoginDatacopy));
                currentLogin.push({username:user});
                localStorage.setItem("currentLoginInfo",JSON.stringify(currentLogin));
            }
            checkotp();
...
function checkusername_password(fetchedData,user,pass){
    for (let i = 0; i <= fetchedData.length-1; i++) {
        var ans=((fetchedData[i].username === user) && (fetchedData[i].password === pass));
    }
    if(ans){
        return ans;
    }else{
...
function checkusername(fetchedData,user){
    for (let i = 0; i <= fetchedData.length-1; i++) {
        var ans=fetchedData[i].username === user;
    }
    return ans;
}
// function setusername(fetchedData,user){
//     for (let i = 0; i <= fetchedData.length-1; i++) {
//         var ans=fetchedData[i].username = user;
//     }
//     return ans;
...
function logincheck(){
    ...
    var fetchedData = JSON.parse(localStorage.getItem("LoginDatainfo"));
    if(user.length ==" " || pass.length == " "){
        ...
        if(fetchedData){
            if (checkusername_password(fetchedData,user,pass)== true){
                currentLogin.push({username:user});
                localStorage.setItem("currentLoginInfo",JSON.stringify(currentLogin));
                ...
                Swal.fire({
                    ...
                    icon: 'info',
                    title: 'Not Matched',
                    text: 'Please SignUp First To Login\n',
                    showConfirmButton: false,
                    timer: 1500
                });
            }
...
function checkotp() {
    var fillotp = $("#otp").val();
    var getotp = localStorage.getItem("GeneratedOTP");
    console.log("hello i m in check otp")
    if (getotp) {
        getotp = parseInt(getotp);
        if (getotp = fillotp) {
            welcomemail();
            delete localStorage.GeneratedOTP;
        } else {
            Swal.fire({
                position: 'center',
                icon: 'error',
...
function sendmail() {
    ...
    Email.send({
        ...
        Password: "kfvytnenvqvskope",
        To: email,
        From: "sellasticverify@gamil.com",
        Subject: "Sellastic One Time Password(OTP)",
        Body: mailbody
    }).then(
        message => {
            if (message == "OK") {
                ...
                Swal.fire({
                    ...
                    text: 'Please Be Sure Your Mail id is Exist',
                    showConfirmButton: false,
                    timer: 1500
                });
            }
							...
							<tr>
								<td align="center" valign="top" style="padding-bottom:30px;padding-left:20px;padding-right:20px;" class="subTitle">
									...
									<h4 class="text" style="color:#999999; font-family:'Open Sans', Helvetica, Arial, sans-serif; font-size:16px; font-weight:600; font-style:normal; letter-spacing:normal; line-height:26px; text-transform:none; text-align:center; padding:0; margin:0">
										${user} Getting Started With Sellastic Shopping
									</h4>
								</td>
							</tr>
